...
def process_excel_report():
    ...
    # Find the Excel file with current date
    expected_filename = f"MART_Trips_{datetime.now().strftime('%Y%m%d')}.xlsx"
    ...
    output_file = os.path.join(downloads_folder, f"Schedule_report_{current_date}.docx")
    ...
    for idx, row in df.iterrows():
        ...
        if "MONITOR" in comments or "MT" in comments:
            df.at[idx, "Name"] = f"{row['Name']} *Monitored"
    ...
    # Add title
    title_run = intro.add_run(f"RELIAMED {formatted_date}, FINAL SCHEDULED TRIPS\n")
    title_run.bold = True
